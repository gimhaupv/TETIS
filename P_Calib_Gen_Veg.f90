...
!
!    !Para que no de error si hay datos de sedimentos (X) en el fichero de episodio
!    IF (config(4)) THEN
!      IF (ksedq.gt.0) THEN
...
CALL iniciaTetis(4,sale) !4, porque se llama desde CalibAutomVeg
if(sale.eq.2)go to 95

artem=TRIM(ADJUSTL(dirtra))//'resumen-SCEUA-VEG.txt'
OPEN (33,file=artem)
...
DO i=1,ncon
    write(*,*)ncon,i,pizq
    !READ(33,'(22F12.4)')pder,(r(j),j=1,9),betappt,(wdad(j,1),j=1,5),bbeta,ro1,ro2,tbase,expinf,alpha
    READ(33,*)pder,(r(j),j=1,9),betappt,(wdad(j,1),j=1,5),bbeta,ro1,ro2,tbase,expinf,alpha,(wdad(j,1),j=7,8)
    DO j=1,cantUsosSuelosVeg
        !READ(33,'(11(F12.4))')(lamb(j,k),k=1,11)
        READ(33,*)(lamb(j,k),k=1,11)
    END DO
...
CALL deletefich(TRIM(ADJUSTL(dirtra))//'resumen-SCEUA-VEG.txt')
...
IF (xchk(10)) THEN
  DO n=1,ncel
    cell(n).h(1)=cell(n).hu2*humr(1)*r(1)/100. !Estamos en config5             !mm
  ENDDO
END IF
IF (xchk(11)) THEN
  DO n=1,ncel
  ...
  DO ia=1,alfafo
	...
	DO isce_veg=1,cantUsosSuelosVeg
        DO jsce_veg=1,11
            IF(xchk_veg(isce_veg,jsce_veg)) THEN
                cont2=cont2+1
                lamb(isce_veg,jsce_veg)=2.0*g(cont2)-b(qvar,cont2)
            END IF
        ENDDO
    ENDDO
	...
	DO isce_veg=1,cantUsosSuelosVeg
        DO jsce_veg=1,11
            IF(xchk_veg(isce_veg,jsce_veg)) THEN
                cont2=cont2+1
                IF (lamb(isce_veg,jsce_veg).lt.xlb_veg(isce_veg,jsce_veg).OR.lamb(isce_veg,jsce_veg).gt.xub_veg(isce_veg,jsce_veg)) THEN
                    !Genera aleatoriamente un número cualquiera
	                CALL RANDOM_NUMBER(nrand)
  	                lamb(isce_veg,jsce_veg)=xlb_veg(isce_veg,jsce_veg)+(xub_veg(isce_veg,jsce_veg)-xlb_veg(isce_veg,jsce_veg))*nrand
	            ENDIF                
            END IF
        ENDDO
    ENDDO
    ...
    DO isce_veg=1,cantUsosSuelosVeg
        DO jsce_veg=1,11
            IF(xchk_veg(isce_veg,jsce_veg)) THEN
                cont2=cont2+1
                xpar(cont,cont2)=lamb(isce_veg,jsce_veg)
            END IF
        ENDDO
    ENDDO
    CALL imprestad_veg
	...
	IF (fobj.lt.b(qvar,nparam+1)) THEN  !es minimizacion, así que es "menor que"
	  ...
	  DO isce_veg=1,cantUsosSuelosVeg
        DO jsce_veg=1,11 
            IF(xchk_veg(isce_veg,jsce_veg)) THEN
                cont2=cont2+1
                b(qvar,cont2)=lamb(isce_veg,jsce_veg)
            END IF
        ENDDO
      ENDDO
      ...
      DO isce_veg=1,cantUsosSuelosVeg
        DO jsce_veg=1,11
            IF (xchk_veg(isce_veg,jsce_veg)) THEN
                cont2=cont2+1
                c(cont2)=(g(cont2)+b(qvar,cont2))/2.0
                lamb(isce_veg,jsce_veg)=c(cont2)
            END IF
        ENDDO
      ENDDO
	  CALL FcnSCE_gen_veg
        ...
        DO isce_veg=1,cantUsosSuelosVeg
            DO jsce_veg=1,11
                IF(xchk_veg(isce_veg,jsce_veg)) THEN
                    cont2=cont2+1
                    CALL RANDOM_NUMBER(nrand)
                    c(cont2)=xlb_veg(isce_veg,jsce_veg)+(xub_veg(isce_veg,jsce_veg)-xlb_veg(isce_veg,jsce_veg))*nrand
                    lamb(isce_veg,jsce_veg)=c(cont2)
                END IF
            ENDDO
        ENDDO
...
!**********************************************************
SUBROUTINE SCEUA_gen_veg
USE modtet
!USE DFLIB
IMPLICIT NONE

ALLOCATE (xguess(24),xlb(24),xub(24),humr(8),xchk(24))
ALLOCATE (xchk_veg(cantUsosSuelosVeg,11),xlb_veg(cantUsosSuelosVeg,11),xub_veg(cantUsosSuelosVeg,11),xguess_veg(cantUsosSuelosVeg,11))

artem=TRIM(ADJUSTL(dirtra))//'Var-SCEUA_HidrVeg.txt' !(Vicente) Var-SCEUA.txt
...
ELSE
  ...
  DO isce=1,24 !(Vicente) Añadimos 2 ultimos FCs
    READ(10,*,err=248)xlb(isce),xub(isce),xguess(isce),xchk(isce)
  ENDDO
  CLOSE(10)
ENDIF

OPEN(10,file=TRIM(ADJUSTL(dirtra))//'xlb_veg-SCEUA.txt',err=249)
DO i=1,cantUsosSuelosVeg
    READ(10,*,err=250)(xlb_veg(i,j),j=1,11)
ENDDO
CLOSE(10)  
OPEN(10,file=TRIM(ADJUSTL(dirtra))//'xub_veg-SCEUA.txt',err=251)
DO i=1,cantUsosSuelosVeg
    READ(10,*,err=252)(xub_veg(i,j),j=1,11)
ENDDO
CLOSE(10) 
OPEN(10,file=TRIM(ADJUSTL(dirtra))//'xguess_veg-SCEUA.txt',err=253)
DO i=1,cantUsosSuelosVeg
    READ(10,*,err=254)(xguess_veg(i,j),j=1,11)
ENDDO
CLOSE(10) 
OPEN(10,file=TRIM(ADJUSTL(dirtra))//'xchk_veg-SCEUA.txt',err=255)
DO i=1,cantUsosSuelosVeg
    READ(10,*,err=256)(xchk_veg(i,j),j=1,11)
ENDDO
CLOSE(10) 
...
DO isce_veg=1,cantUsosSuelosVeg
    DO jsce_veg=1,11
        IF(xchk_veg(isce_veg,jsce_veg)) nparam=nparam+1
    ENDDO
ENDDO
  ...
  DO isce_veg=1,cantUsosSuelosVeg
    DO jsce_veg=1,11
        IF(xchk_veg(isce_veg,jsce_veg)) THEN
	          cont2=cont2+1
	          CALL RANDOM_NUMBER(nrand)  !ksce numero de fila, cont2 numero de columna
	          dvar(ksce,cont2)=xlb_veg(isce_veg,jsce_veg)+(xub_veg(isce_veg,jsce_veg)-xlb_veg(isce_veg,jsce_veg))*nrand ! crea una matriz dove per ogni parametro a calibrare si generano e salvano 's'=sval valori aleatori di tentativo
	    ENDIF
	ENDDO
  ENDDO
ENDDO
...
DO isce_veg=1,cantUsosSuelosVeg
    DO jsce_veg=1,11
        IF(xchk_veg(isce_veg,jsce_veg)) THEN
	        cont2=cont2+1
	        dvar(1,cont2)=xguess_veg(isce_veg,jsce_veg)
	    ENDIF
	ENDDO
ENDDO
...
DO ksce=1,svar
  ...
  DO isce=1,9                   !Para factores correctores r(9)
	...
	ENDIF
  ENDDO
  DO isce=10,14               !Para humedad inicial  h(6)ù
    IF (xchk(isce)) THEN 
      cont2=cont2+1
  ...
  DO isce_veg=1,cantUsosSuelosVeg
    DO jsce_veg=1,11
        IF(xchk_veg(isce_veg,jsce_veg)) THEN
	        cont2=cont2+1
	        lamb(isce_veg,jsce_veg) = dvar(ksce,cont2)	        
	    ENDIF
	ENDDO
 ENDDO
...
DO WHILE (pmin.lt.pvar)
  ...
  DO isce_veg=1,cantUsosSuelosVeg
    DO jsce_veg=1,11 
        IF(xchk_veg(isce_veg,jsce_veg)) THEN
            WRITE(*,'(A9,I2,A5,I2,A2,F15.5)')'Fact. Use',isce_veg,' lamb',jsce_veg,': ',lamb(isce_veg,jsce_veg)
        END IF
    ENDDO
  ENDDO
  ...
  DO i=1,cantUsosSuelosVeg
    WRITE(33,'(11(F12.4))')(lamb(i,j),j=1,11)
  END DO  
...
246 mensaje=strings(315)
errr=1
CALL errores
GOTO 94
248 mensaje=strings(316)
errr=1
CALL errores
GOTO 94
249 mensaje=strings(317)
errr=1
CALL errores
GOTO 94
250 mensaje=strings(318)
errr=1
CALL errores
GOTO 94
251 mensaje=strings(319)
errr=1
CALL errores
GOTO 94
252 mensaje=strings(320)
errr=1
CALL errores
GOTO 94
253 mensaje=strings(321)
errr=1
CALL errores
GOTO 94
254 mensaje=strings(322)
errr=1
CALL errores
GOTO 94
255 mensaje=strings(323)
errr=1
CALL errores
GOTO 94
256 mensaje=strings(167)
errr=1
CALL errores
GOTO 94
