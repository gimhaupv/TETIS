...
IF (existe)THEN
    ...
    DO i=1,nman
       READ (15,*,err=216) manantial(i).name,manantial(i).utmx,manantial(i).utmy,manantial(i).coef
...
216 IF (lang.eq.1) THEN
        mensaje='056 Lectura de datos errónea en '//TRIM(ADJUSTL(arch(37)))
    ELSE IF (lang.eq.2) THEN
        mensaje='056 Error reading '//TRIM(ADJUSTL(arch(37)))
    ENDIF
    errr=1
CALL errores
GOTO 94
